...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Direct Local Hostinger SSD Storage (Primary) + MySQL Persistent DB Backup
    try {
      ...
      try {
        ...
        await mysqlSaveMediaUpload(cleanId, 'reviews', cleanId, contentType, buffer);
      } catch (dbErr: any) {
      ...
      console.warn('Local review file upload error:', localErr?.message);
    }
